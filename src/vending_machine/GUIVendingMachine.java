...
public class GUIVendingMachine extends JFrame {
    ...
    public GUIVendingMachine(String s) {
        ...
        //To refill all items
        JButton refillItems = new JButton("Refill all items");
        refillItems.addActionListener(new ActionListener() {
        	
        	@Override
			public void actionPerformed(ActionEvent arg0) {
        		for(int i=0; i<atm.itemsInMachine.length; i++) {
        			atm.quantity[i] = 5;
                	tableWithQuantity.setValueAt(atm.quantity[i], i, 1);
        		}
